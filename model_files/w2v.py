...
def load_w2v_embedding(word_list, uniform_scale, dimension_size):
    embed_file = r'embedding\uzbek-word2vec.bin'
...
def load_glove_embedding(word_list, uniform_scale, dimension_size):
    ...
    for word in word_list:
        if word in word2offset:
            line = linecache.getline(r'embedding\uzbek_word2vec_300d.txt', word2offset[word]+1)
...
def load_yelp_embedding(word_list, uniform_scale, dimension_size):
    word2embed = {}
    with open(r'embedding\uzbek_yelp_embedding.txt', 'r') as fopen:
        for line in fopen:
            w = line.split(sep=' ')
...
def save_embedding(word_list, word_embedding,
                   word_list_file=r'embedding\uz_yelp_words.txt',
                   word_embedding_file=r'embedding\uzbek_yelp_embedding.txt'):
